...
const SPAGHETTI_PATH = `/${SPAGHETTI_QUIZ_ID}`;

export function generateStaticParams() {
  return getQuizLocales(SPAGHETTI_QUIZ_ID).map((locale) => ({ locale }));
}

export default async function SpaghettiPage({
  params,
}: PageProps<"/[locale]/spaghetti">) {
  const { locale } = await params;

  if (!isValidLocale(locale)) notFound();

  const quiz = getQuiz(SPAGHETTI_QUIZ_ID, locale);
  if (!quiz) notFound();

  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col px-5 py-6 sm:px-8 sm:py-8">
      <SiteHeader locale={locale} path={SPAGHETTI_PATH} />
