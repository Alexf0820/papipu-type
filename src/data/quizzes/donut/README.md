...
2. Run `npm run import-quiz-content -- donut`. It validates all input before replacing `definition.ts`, `ja.ts`, `en.ts`, and both result files.
3. Keep each visual key as `donut-<result-type>`; after image handoff, manually add the VisualKey union, Body, visual registry entry, and faceTransform.
...
6. Run `npm run validate-quiz -- donut` and `npm run verify-quiz -- donut` before production review.
